...
std::optional<std::string> atspi_utils::GetPrimaryText(AtspiAccessible* element) {
  ...
  gint num_chars = atspi_text_get_character_count(text_interface, &char_count_error);
  ...
  for (unsigned int i = 0; i < n_selections; i++) {
    ...
    if (range) {
      gchar* selection_text =
          atspi_text_get_text(text_iface, range->start_offset, range->end_offset, &error);
      if (error) {
        g_error_free(error);
...
std::optional<AtspiPoint*> atspi_utils::GetPosition(AtspiAccessible* element) {
  ...
  AtspiPoint* coords =
      atspi_component_get_position(component, ATSPI_COORD_TYPE_SCREEN, &get_position_error);
...
std::optional<AtspiAccessible*> atspi_utils::GetChildAtIndex(AtspiAccessible* element,
  ...
  AtspiAccessible* child =
      atspi_accessible_get_child_at_index(element, child_index, &get_child_error);
