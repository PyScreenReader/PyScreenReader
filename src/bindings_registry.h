...
class BindingsRegistry {
 public:
  /**
   * BindingFunc is a function that consumes a PyBinding module, and perform
   ...
   *
   * This method adds a new binding function to the registry, associating it
   * with a name and a list of dependencies (other bindings that must be run
   * before this one).
   *
   * @param name The unique name identifying the binding. Must not conflict with
   * existing names.
   * @param func The function that performs the actual binding to Python (e.g.,
   * class/module binding).
   * @param deps A list of names this binding depends on. These dependencies
   ...
   *
   * This method checks that all dependencies are valid, then performs a
   * topological sort to ensure that bindings are run in a correct order (i.e.,
   * all dependencies are bound before the dependent). It invokes each binding
   * function, passing the given Python module as a parameter.
   *
   * @param m The pybind11 module into which all bindings will be defined.
   *
   * @throws std::invalid_argument if a dependency is undefined or if there is a
 ...
 private:
   ...
   *
   * @throws std::invalid_argument if a dependency is referenced but never
