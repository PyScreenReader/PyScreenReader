...
def test_repr_str(widget: VirtualButtonWidget):
    expected_str = "{VirtualButtonWidget, title=, help=, x=0, y=0, width=0, height=0, visible=false}"
    ...
    expected_str = "{VirtualButtonWidget, title=title, help=help, x=1, y=2, width=3, height=4, visible=true}"
    assert str(widget) == expected_str
    assert repr(widget) == expected_str
